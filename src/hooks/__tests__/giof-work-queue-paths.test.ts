...
describe("filtros GIOF en las colas existentes", () => {
  it("integra mine/all y assignee sin reemplazar los filtros de cada cola", () => {
    expect(
      getRequestsPath({
        scope: "review",
        work_scope: GIOF_WORK_SCOPE.MINE,
        page: 2,
      }),
    ).toContain("scope=review&work_scope=mine");
    expect(
      getPaymentQueuePath({
        status: "APPROVED",
        work_scope: GIOF_WORK_SCOPE.ALL,
      }),
    ).toContain("status=APPROVED&work_scope=all");
    expect(
      getRenditionsPath({
        work_scope: GIOF_WORK_SCOPE.ASSIGNEE,
        assignee_id: "user-1",
      }),
    ).toContain("work_scope=assignee&assignee_id=user-1");
  });

  it("serializa all explícito como query canónica en los tres pools", () => {
    expect(
      getRequestsPath({ scope: "review", work_scope: GIOF_WORK_SCOPE.ALL }),
    ).toContain("scope=review&work_scope=all");
    expect(getPaymentQueuePath({ work_scope: GIOF_WORK_SCOPE.ALL })).toContain(
      "work_scope=all",
    );
    expect(getRenditionsPath({ work_scope: GIOF_WORK_SCOPE.ALL })).toContain(
      "work_scope=all",
    );
  });
  ...
  it("reintenta sin filtros GIOF solo cuando un backend anterior los rechaza", async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(
      1,
      "/requests?page=1&scope=review&work_scope=mine",
    );
  ...
  it("no oculta otros errores 400 del backend", async () => {
    ...
    await expect(
      fetchGiofCompatibleQueue(() => Promise.reject(error), fallback),
    ).rejects.toBe(error);
