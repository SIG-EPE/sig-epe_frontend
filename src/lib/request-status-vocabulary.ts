...
const PAYMENT_STATUS_LABELS: Readonly<Partial<Record<RequestStatus, string>>> =
  {
    ...
    [REQUEST_STATUS.REJECTED]: "Pago rechazado",
  };

const DASHBOARD_STATUS_LABELS: Readonly<
  Partial<Record<RequestStatus, string>>
> = {
...
const RENDITION_LIFECYCLE_LABELS: Readonly<
  Partial<Record<RequestStatus, string>>
> = {
...
function getContextualLabel(
  ...
  if (surface === REQUEST_STATUS_SURFACE.REVIEW)
    return REVIEW_STATUS_LABELS[status];
  if (surface === REQUEST_STATUS_SURFACE.PAYMENT)
    return PAYMENT_STATUS_LABELS[status];
  if (surface === REQUEST_STATUS_SURFACE.DASHBOARD)
...
export function formatRequestStatus(
  ...
  return (
    getContextualLabel(requestStatus, context.surface) ??
...
export function formatRenditionQueueStatus(status: string): string {
  return (
    RENDITION_QUEUE_STATUS_LABELS[status as RenditionStatus] ??
    `Estado no reconocido (${status})`
  );
}
