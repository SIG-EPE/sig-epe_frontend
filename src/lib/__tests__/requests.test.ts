...
import {
  ...
  rejectApprovedPayment,
  parsePaymentRejectionProjection,
...
describe("request review URL contract", () => {
  ...
  it("round-trips the exact review allowlist in canonical order without sort", () => {
    const parsed = parseRequestReviewUrl(
      new URLSearchParams(
    ...
    expect(serializeRequestReviewUrl(parsed.filters).has("direction")).toBe(
      false,
    );
  });
  ...
  it("omits defaults and resets page when a filter changes", () => {
    ...
    const updated = updateRequestReviewUrl(
      new URLSearchParams(
        "scope=review&page=4&limit=20&status=SUBMITTED&sort=OLDEST_FIRST",
      ),
...
describe("isRequestStateConflict", () => {
  it("detecta solo ApiRequestError 409 con el código estable", () => {
    expect(
      isRequestStateConflict(
        new ApiRequestError(409, REQUEST_STATE_CONFLICT_BODY),
      ),
    ).toBe(true);
    expect(
      isRequestStateConflict(
        new ApiRequestError(400, REQUEST_STATE_CONFLICT_BODY),
      ),
    ).toBe(false);
    expect(
      isRequestStateConflict(
        new ApiRequestError(409, {
          ...REQUEST_STATE_CONFLICT_BODY,
          code: "OTHER",
        }),
      ),
    ).toBe(false);
...
describe("rejectApprovedPayment client", () => {
  it("serializes the mandatory reason and PAYMENT lease headers", async () => {
    const response = {
      id: "request-1",
      status: REQUEST_STATUS.REJECTED,
    } as PaymentRequest;
    const post = vi.spyOn(api, "post").mockResolvedValueOnce(response);

    await expect(
      rejectApprovedPayment(
        "request-1",
        { reason: "  Banco rechazó la transferencia  " },
        { assignmentVersion: "4", token: "lease-token" },
      ),
    ).resolves.toBe(response);

    expect(post).toHaveBeenCalledWith(
      "/requests/request-1/reject-payment",
      { reason: "Banco rechazó la transferencia" },
      {
        headers: {
          "x-giof-assignment-version": "4",
  ...
  it("rejects blank reasons locally and preserves ApiRequestError responses", async () => {
    await expect(
      rejectApprovedPayment(
        "request-1",
        { reason: "   " },
    ...
    const conflict = new ApiRequestError(409, {
      statusCode: 409,
      code: "PAYMENT_EXECUTION_EXISTS",
      message: "Conflict",
      error: "Conflict",
      timestamp: "2026-09-15T00:00:00.000Z",
      path: "/requests/request-1/reject-payment",
    });
    const post = vi.spyOn(api, "post").mockRejectedValueOnce(conflict);
    await expect(
      rejectApprovedPayment(
        "request-1",
...
describe("payment rejection DTO projection", () => {
  it("parses only the public rejection fields and drops actor PII", () => {
    expect(
      parsePaymentRejectionProjection({
        reason: "Cuenta cerrada",
      ...
      reason: "Cuenta cerrada",
      rejected_at: "2026-09-15T18:00:00.000Z",
      actor: { id: "user-1", first_name: "Ana", last_name: "Pérez" },
    });
  });

  it("rejects an incomplete projection and maps payment rejection conflicts", () => {
    expect(
      parsePaymentRejectionProjection({
        reason: "Rechazo de revisión",
        rejected_at: "2026-09-15T18:00:00.000Z",
      }),
    ).toBeNull();
    ...
    expect(
      getApiErrorMessages(
        new ApiRequestError(409, {
          statusCode: 409,
          code: "PAYMENT_REJECTION_STATUS_CONFLICT",
          message: "Conflict",
          error: "Conflict",
          timestamp: "2026-09-15T00:00:00.000Z",
          path: "/requests/request-1/reject-payment",
...
function makeHistoryItem(
  overrides: Partial<RequestStatusHistoryItem>,
...
function makePreview(
  overrides: Partial<RequestBudgetPreview>,
...
describe("requests helpers", () => {
  it("etiqueta todos los tipos del contrato y mantiene Rexan fuera de opciones iniciales", () => {
    ...
    expect(REQUEST_TYPE_OPTIONS.map((option) => option.value)).not.toContain(
      REQUEST_TYPE.ADVANCE_SETTLEMENT,
    );
  });
  ...
  it.each([
    ["SUPPLIER_IDENTITY_REQUIRED", "data", "proveedor"],
    ["SUPPLIER_IDENTITY_INVALID", "data", "proveedor"],
    ...
    ["REIMBURSEMENT_RECEIPT_REQUIRED", "documents", "comprobante"],
  ])(
    "traduce y dirige el código real %s sin depender del texto",
    (code, step, text) => {
      const error = new ApiRequestError(400, {
        statusCode: 400,
        code,
        message: "A supplier document is required.",
        error: "Bad Request",
        timestamp: "2026-09-11T00:00:00.000Z",
        path: "/requests/r/submit",
      });
      expect(getApiErrorMessages(error)[0]).toContain(text);
      expect(getRequestErrorStep(error)).toBe(step);
      expect(getMissingDocumentMessagesFromError(error)).toEqual(
        step === "documents" ? getApiErrorMessages(error) : [],
      );
      expect(error.body.code).toBe(code);
    },
  );

  it("mantiene el fallback documental y no inventa pasos para errores desconocidos", () => {
    expect(getRequestErrorStep(new Error("Falta adjuntar Excel PxQ."))).toBe(
      "documents",
    );
    expect(getRequestErrorStep(new Error("Conflicto desconocido"))).toBeNull();
    expect(getApiErrorMessages(new Error("Conflicto desconocido"))).toEqual([
      "Conflicto desconocido",
    ]);
  });

  it("conserva moneda nullable al leer sin etiquetar el principal como soles", () => {
    const request = makeRequest({ currency: null });
    expect(
      formatRequestCurrency(request.requested_amount, request.currency),
    ).toContain("Moneda pendiente de resolución");
    expect(deriveRexanApprovalPreview(100, 90, request.currency)).toBeNull();
  });
  ...
  it("habilita inicio de rendición solo para anticipos pagados y usuarios autorizados", () => {
    const paidAdvance = makeRequest({
      status: REQUEST_STATUS.PAID,
      requester_id: "user-1",
    });
    const paidReimbursement = makeRequest({
      request_type: REQUEST_TYPE.REIMBURSEMENT,
      status: REQUEST_STATUS.PAID,
    });
    const paidSupplierPayment = makeRequest({
    ...
    expect(
      canStartAdvanceSettlement(
        ROLE_CODE.SOLICITANTE_EPE,
        makeRequest({ status: REQUEST_STATUS.APPROVED }),
        "user-1",
      ),
    ).toBe(false);
    expect(
  ...
  it("deriva CTA de rendición para iniciar, continuar, reintentar y marcar completada", () => {
    ...
    expect(
      getAdvanceSettlementCta(ROLE_CODE.SOLICITANTE_EPE, paidAdvance, "user-1"),
    ).toMatchObject({
      state: "can-start",
    ...
    expect(
      getAdvanceSettlementCta(
        ...
        makeRequest({
          status: REQUEST_STATUS.PAID,
          advanceSettlements: [draftSettlement],
        }),
        "user-1",
      ),
    ).toMatchObject({
    ...
    expect(
      getAdvanceSettlementCta(
        ...
        makeRequest({
          status: REQUEST_STATUS.PAID,
          advanceSettlements: [submittedSettlement],
        }),
        "user-1",
      ),
    ).toMatchObject({
    ...
    expect(
      getAdvanceSettlementCta(
        ...
        makeRequest({
          status: REQUEST_STATUS.PAID,
          advanceSettlements: [rejectedSettlement],
        }),
        "user-1",
      ),
    ).toMatchObject({
    ...
    expect(
      getAdvanceSettlementCta(
        ...
        makeRequest({
          status: REQUEST_STATUS.PAID,
          advanceSettlements: [approvedSettlement],
        }),
        "user-1",
      ),
    ).toMatchObject({
  ...
  it("aclara el siguiente paso de rendición según responsable", () => {
    ...
    expect(
      getRenditionNextStepGuidance(
        ROLE_CODE.GIOF_GESTOR,
        makeRequest({ status: REQUEST_STATUS.PAID, requester_id: "giof-1" }),
        "giof-1",
      ),
    ).toMatchObject({
      title: "Siguiente paso: preparar rendición",
  ...
  it("etiqueta y deriva estados de bandeja de rendiciones", () => {
    ...
    const pendingDueSoon = makeRenditionRow({
      scheduled_rendition_at: "2026-05-20",
      days_until_due: 5,
      days_remaining: 5,
    });
    const overdue = makeRenditionRow({
    ...
    const overdueAdvance = makeRequest({
      status: REQUEST_STATUS.PAID,
      scheduled_rendition_at: "2026-05-14",
    });
    ...
    try {
      expect(parseRenditionStatusFilter("OVERDUE")).toBe(
        RENDITION_STATUS.OVERDUE,
      );
      expect(parseRenditionSortField("paid_at")).toBe(
        RENDITION_SORT_FIELD.PAID_AT,
      );
      expect(parseRenditionSortField(null)).toBe(
        RENDITION_SORT_FIELD.LAST_ACTIVITY,
      );
      expect(parseRenditionSortDirection("desc")).toBe(
        RENDITION_SORT_DIRECTION.DESC,
      );
      expect(parseRenditionSortDirection(null)).toBe(
        RENDITION_SORT_DIRECTION.DESC,
      );
      expect(getRenditionStatusLabel(RENDITION_STATUS.IN_REVIEW)).toBe(
        "En revisión",
      );
      expect(getRenditionDueLabel(overdue)).toBe("Vencida hace 2 días");
      expect(
        getRenditionSummaryCount(
          { key: "due-soon", label: "Próximas a vencer", description: "" },
          counts,
          [pendingDueSoon, overdue],
        ),
      ).toBe(1);
      expect(getRenditionDueLabel(pendingDueSoon)).toBe("5 días para vencer");
      expect(getPaymentRequestRenditionStatus(paidAdvance)).toBe(
      ...
      expect(
        getPaymentRequestRenditionStatus(
          makeRequest({
            status: REQUEST_STATUS.PAID,
            advanceSettlements: [observedSettlement],
          }),
        ),
      ).toBe(RENDITION_STATUS.OBSERVED);
      expect(
        getPaymentRequestRenditionStatus(
          makeRequest({
            status: REQUEST_STATUS.PAID,
      ...
      ).toBe(RENDITION_STATUS.PENDING);
      expect(
        getPaymentRequestRenditionStatus({
          ...overdueAdvance,
          advanceSettlements: [rejectedSettlement],
        }),
      ).toBe(RENDITION_STATUS.OVERDUE);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("formats request dates and due labels in Lima business time", () => {
    ...
    expect(formatRequestDateTime("2026-06-01T05:00:00.000Z")).toContain(
      "1 jun",
    );

    const pendingDueToday = makeRenditionRow({
      scheduled_rendition_at: "2026-05-31",
      days_until_due: 0,
      days_remaining: 0,
    });
  ...
  it("formatea los seis estados canónicos sin convertir una fecha concreta en ausencia de plazo", () => {
    const cases = [
      ...
      [
        RENDITION_DEADLINE_STATE.OVERDUE,
        "2026-05-13",
        -2,
        "Vencida hace 2 días",
      ],
  ...
  it("aplica al card due-soon solo OPEN entre 1 y 15 y separa DUE_TODAY", () => {
    const row = (
      deadlineState: (typeof RENDITION_DEADLINE_STATE)[keyof typeof RENDITION_DEADLINE_STATE],
      days: number | null,
    ) =>
      makeRenditionRow({
        deadline_date: "2026-05-20",
    ...
    expect(isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.DUE_TODAY, 0))).toBe(
      false,
    );
    expect(isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.OPEN, 1))).toBe(
      true,
    );
    expect(isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.OPEN, 15))).toBe(
      true,
    );
    expect(isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.OPEN, 16))).toBe(
      false,
    );
    expect(isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.OVERDUE, -1))).toBe(
      false,
    );
    expect(
      isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.PRESENTED, null)),
    ).toBe(false);
    expect(
      isRenditionDueSoon(row(RENDITION_DEADLINE_STATE.COMPLETED, null)),
    ).toBe(false);
  });
  ...
  it("aplica mensaje de bloqueo de nuevo anticipo sin bloquear corrección REXAN", () => {
    const backendError = new ApiRequestError(400, {
      statusCode: 400,
      message:
        "Tienes 2 anticipos pagados pendientes de rendición antes de crear un nuevo anticipo.",
      error: "Bad Request",
    ...
    expect(
      getNewAdvancePendingSettlementBlockMessage(REQUEST_TYPE.ADVANCE, null),
    ).toContain("dos o más anticipos pagados pendientes de rendición");
    expect(
      getNewAdvancePendingSettlementBlockMessage(
        REQUEST_TYPE.ADVANCE_SETTLEMENT,
        backendError,
  ...
  it("valida archivos adjuntos permitidos para solicitudes", () => {
    const validPdf = new File(["contenido"], "sustento.pdf", {
      type: "application/pdf",
    });
    const validJpeg = new File(["contenido"], "foto.jpg", {
      type: "image/jpeg",
    });
    const validPng = new File(["contenido"], "captura.png", {
      type: "image/png",
    });
    const emptyPdf = new File([], "vacio.pdf", { type: "application/pdf" });
    const textFile = new File(["texto"], "nota.txt", { type: "text/plain" });
    const excelFile = new File(["contenido"], "pxq.xlsx", {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    const excelFileWithoutMime = new File(["contenido"], "pxq.xls", {
      type: "",
    });
    ...
    expect(
      validateRequestDocumentFile(excelFile, REQUEST_DOCUMENT_CATEGORY.PXQ),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(
        excelFileWithoutMime,
        REQUEST_DOCUMENT_CATEGORY.SETTLEMENT_REPORT,
      ),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(validPdf, REQUEST_DOCUMENT_CATEGORY.PXQ),
    ).toContain("Formato no permitido");
    expect(
      validateRequestDocumentFile(excelFile, REQUEST_DOCUMENT_CATEGORY.RECEIPT),
    ).toContain("Formato no permitido");
    expect(validateRequestDocumentFile(emptyPdf)).toContain("vacío");
    expect(validateRequestDocumentFile(textFile)).toContain(
      "Formato no permitido",
    );
    expect(validateRequestDocumentFile(oversizedFile)).toContain("10 MB");
    expect(validateRequestDocumentFile(null)).toContain(
      "Selecciona un archivo",
    );
  });

  it("aplica extensión y MIME por categoría documental de proveedor", () => {
    const pdf = new File(["pdf"], "documento.pdf", { type: "application/pdf" });
    const jpg = new File(["jpg"], "documento.jpg", { type: "image/jpeg" });
    const xlsx = new File(["xlsx"], "documento.xlsx", {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    const docx = new File(["docx"], "documento.docx", {
      type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    });
    const zip = new File(["zip"], "documento.zip", { type: "application/zip" });
    const disguisedContract = new File(["jpg"], "contrato.pdf", {
      type: "image/jpeg",
    });

    expect(
      validateRequestDocumentFile(jpg, REQUEST_DOCUMENT_CATEGORY.QUOTATION),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(pdf, REQUEST_DOCUMENT_CATEGORY.CONTRACT),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(xlsx, REQUEST_DOCUMENT_CATEGORY.EVIDENCE),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(docx, REQUEST_DOCUMENT_CATEGORY.EVIDENCE),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(zip, REQUEST_DOCUMENT_CATEGORY.OTHER),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(jpg, REQUEST_DOCUMENT_CATEGORY.INVOICE),
    ).toBeNull();
    expect(
      validateRequestDocumentFile(
        disguisedContract,
        REQUEST_DOCUMENT_CATEGORY.CONTRACT,
      ),
    ).toContain("Formato no permitido");
    expect(
      validateRequestDocumentFile(xlsx, REQUEST_DOCUMENT_CATEGORY.QUOTATION),
    ).toContain("Formato no permitido");
  });

  it("define estados activos de revisión y valida constancias de pago PDF o imagen", () => {
    const validPdf = new File(["contenido"], "telecredito.pdf", {
      type: "application/pdf",
    });
    const validPdfWithoutMime = new File(["contenido"], "telecredito.pdf", {
      type: "",
    });
    const validJpeg = new File(["contenido"], "telecredito.jpg", {
      type: "image/jpeg",
    });
    const validJpegWithoutMime = new File(["contenido"], "telecredito.jpeg", {
      type: "",
    });
    const validPng = new File(["contenido"], "telecredito.png", {
      type: "image/png",
    });
    const textFile = new File(["texto"], "telecredito.txt", {
      type: "text/plain",
    });
    const oversizedFile = new File(
      [new Uint8Array(10 * 1024 * 1024 + 1)],
  ...
  it("formatea metadatos de documentos adjuntos", () => {
    ...
    expect(
      getRequestDocumentCategoryLabel(REQUEST_DOCUMENT_CATEGORY.RECEIPT),
    ).toBe("Comprobante");
    expect(
      getRequestDocumentCategoryLabel(
        REQUEST_DOCUMENT_CATEGORY.REQUEST_SUPPORT,
      ),
    ).toBe("Sustento de solicitud");
    expect(
      getRequestDocumentCategoryLabel(REQUEST_DOCUMENT_CATEGORY.RETURN_PROOF),
    ).toBe("Constancia de devolución");
    expect(getRequestDocumentCategoryLabel("RAW_UNKNOWN")).toBe(
      "Categoría no reconocida",
    );
    expect(
      getRequestDocumentUploadStatusLabel(
        REQUEST_DOCUMENT_UPLOAD_STATUS.PERMANENT,
      ),
    ).toBe("Guardado");
    expect(getRequestDocumentUploadStatusLabel("RAW_UNKNOWN")).toBe(
      "Estado no reconocido",
    );
    expect(
      getRequestDocumentStorageProviderLabel(
        REQUEST_DOCUMENT_STORAGE_PROVIDER.DRIVE,
      ),
    ).toBe("Google Drive");
    expect(getRequestDocumentStorageProviderLabel("RAW_UNKNOWN")).toBe(
    ...
    expect(
      getRequestDocumentAcceptedFormatsLabel(REQUEST_DOCUMENT_CATEGORY.PXQ),
    ).toBe("XLS o XLSX");
    expect(
      getRequestDocumentAcceptedFormatsLabel(REQUEST_DOCUMENT_CATEGORY.RECEIPT),
    ).toBe("PDF, JPG o PNG");
    expect(
      getRequestDocumentAccept(REQUEST_DOCUMENT_CATEGORY.PXQ),
    ).not.toContain("application/pdf");
    expect(
      getRequestDocumentAccept(REQUEST_DOCUMENT_CATEGORY.SETTLEMENT_REPORT),
    ).toContain(".xlsx");
    expect(
      getRequestDocumentAccept(REQUEST_DOCUMENT_CATEGORY.RETURN_PROOF),
    ).toContain("application/pdf");
  });

  it.each([
    ["PlanificaciÃ³n aÃ±o.xlsx", "Planificación año.xlsx"],
    ["Precio Â£.pdf", "Precio £.pdf"],
    ["Cotización del año.pdf", "Cotización del año.pdf"],
    ["reporte-final_2026.pdf", "reporte-final_2026.pdf"],
    ["契約書_日本語.pdf", "契約書_日本語.pdf"],
    ["Símbolo Ã aislado.pdf", "Símbolo Ã aislado.pdf"],
    ["João e Ângela.pdf", "João e Ângela.pdf"],
  ])(
    "normaliza el nombre documental %s solo cuando hay mojibake probable",
    (originalFilename, expectedFilename) => {
      expect(
        getRequestDocumentDisplayName(
          makeDocument({ original_filename: originalFilename }),
        ),
  ...
  it("usa la clasificación retryable explícita del backend para fallos de Drive", () => {
    ...
    expect(isRetryableRequestDocumentUploadError(permanentDriveFailure)).toBe(
      false,
    );
    expect(isRetryableRequestDocumentUploadError(transientDriveFailure)).toBe(
      true,
    );
  });

  it("deriva resultado REXAN y valida constancia solo para devolución", () => {
    const returnProof = makeDocument({
      id: "return-proof-1",
      document_category: REQUEST_DOCUMENT_CATEGORY.RETURN_PROOF,
    });
    const receipt = makeDocument({
      id: "receipt-1",
      document_category: REQUEST_DOCUMENT_CATEGORY.RECEIPT,
    });

    expect(deriveRexanPreviewOutcome(100, 100)).toBe(REXAN_OUTCOME.EXACT);
    expect(deriveRexanPreviewOutcome("100.004", 100.004)).toBe(
      REXAN_OUTCOME.EXACT,
    );
    ...
    expect(
      deriveRexanApprovalPreview(100, 100, REQUEST_CURRENCY.PEN),
    ).toMatchObject({
    ...
    expect(
      deriveRexanApprovalPreview(100, 80, REQUEST_CURRENCY.PEN),
    ).toMatchObject({
      outcome: REXAN_OUTCOME.DEVOLUCION,
      balanceAmount: 20,
    });
    expect(
      deriveRexanApprovalPreview(100, 80, REQUEST_CURRENCY.PEN)?.message,
    ).toMatch(/Devolución: se debe registrar constancia por S\/\s*20\.00\./);
    expect(
      deriveRexanApprovalPreview(100, 125.555, REQUEST_CURRENCY.PEN),
    ).toMatchObject({
      outcome: REXAN_OUTCOME.EXCESS,
      balanceAmount: 25.56,
    });
    expect(
      deriveRexanApprovalPreview(100, 125.555, REQUEST_CURRENCY.PEN)?.message,
    ).toMatch(
      /Saldo adicional por pagar: S\/\s*25\.56 pasará a Cola de Pagos\./,
    );
    expect(getRexanOutcomeLabel(REXAN_OUTCOME.EXCESS)).toBe("Saldo a pagar");
    expect(getReturnProofDocuments([receipt, returnProof])).toEqual([
      returnProof,
    ]);
    expect(
      validateRexanReturnProofSelection(REXAN_OUTCOME.EXACT, undefined, []),
    ).toBeNull();
    expect(
      validateRexanReturnProofSelection(REXAN_OUTCOME.DEVOLUCION, undefined, [
        returnProof,
      ]),
    ).toContain("Selecciona una constancia");
    expect(
      validateRexanReturnProofSelection(REXAN_OUTCOME.DEVOLUCION, receipt.id, [
        receipt,
        returnProof,
      ]),
    ).toContain("constancia de devolución");
    expect(
      validateRexanReturnProofSelection(
  ...
  it("no marca Datos como completado si faltan datos requeridos para envío", () => {
    const steps = getRequestEditStepperItems(REQUEST_EDIT_STEP.DOCUMENTS, {
  ...
  it("valida datos obligatorios de beneficiario y banco antes de revisión/envío", () => {
    const messages = validateRequestDataForSubmit(
      makeRequest({
  ...
  it("impide entrar a revisión cuando faltan datos obligatorios", () => {
    ...
    const result = getRequestReviewNavigationIssues(
      makeRequest({
        budget_planning_line_id: null,
        beneficiary_name: " ",
        beneficiary_document_type: null,
    ...
    expect(result.dataIssues.map((issue) => issue.message)).toEqual(
      expect.arrayContaining([
        "Selecciona una línea POA.",
        "Ingresa el nombre del beneficiario.",
        "Selecciona el banco del beneficiario.",
  ...
  it("impide entrar a revisión cuando falta el comprobante de reembolso", () => {
  ...
  it("expone validaciones con campo y paso para mostrarlas cerca del formulario", () => {
    const issues = validateRequestDataForSubmitIssues(
      makeRequest({
        budget_planning_line_id: null,
        bank_code: null,
        bank_account: null,
  ...
  it("aplica salida segura para errores técnicos desconocidos fuera de ApiRequestError", () => {
    expect(
      getApiErrorMessages(new Error("raw_backend_field should not be empty")),
    ).toEqual([
      "Revisa los datos ingresados. Hay un campo obligatorio o con formato inválido.",
    ]);
  });
  ...
  it("evalúa checklist de documentos requeridos por tipo", () => {
    const advanceMissing = getRequiredDocumentChecklist(
      REQUEST_TYPE.ADVANCE,
      [],
    );
    expect(advanceMissing.isComplete).toBe(false);
    expect(advanceMissing.missingMessages).toEqual([
      "Falta adjuntar Excel PxQ.",
    ]);
    ...
    const reimbursement = getRequiredDocumentChecklist(
      REQUEST_TYPE.REIMBURSEMENT,
      [
      ...
      makeRequest({ request_type: REQUEST_TYPE.REIMBURSEMENT }),
      [{ request_id: "req-1", document_id: "doc-1", receipt_type: "INVOICE" }],
    );
    expect(reimbursement.missingMessages).toEqual([]);

    const supplier = getRequiredDocumentChecklist(
      REQUEST_TYPE.SUPPLIER_PAYMENT,
      [],
      makeRequest({
        request_type: REQUEST_TYPE.SUPPLIER_PAYMENT,
    ...
    expect(supplier.missingMessages).toEqual([
    ...
    const rexanWrongReport = getRequiredDocumentChecklist(
      REQUEST_TYPE.ADVANCE_SETTLEMENT,
      [
        makeDocument({
          document_category: REQUEST_DOCUMENT_CATEGORY.SETTLEMENT_REPORT,
          original_filename: "reporte.pdf",
          safe_filename: "reporte.pdf",
          mime_type: "application/pdf",
        }),
        makeDocument({ document_category: REQUEST_DOCUMENT_CATEGORY.RECEIPT }),
      ],
    ...
    const rexanComplete = getRequiredDocumentChecklist(
      REQUEST_TYPE.ADVANCE_SETTLEMENT,
      [
        makeDocument({
          document_category: REQUEST_DOCUMENT_CATEGORY.SETTLEMENT_REPORT,
          original_filename: "rexan.xlsx",
          safe_filename: "rexan.xlsx",
          mime_type:
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        }),
        makeDocument({ document_category: REQUEST_DOCUMENT_CATEGORY.RECEIPT }),
  ...
  it("aplica heurística frontend para gestionar documentos", () => {
    ...
    const observed = makeRequest({
      status: REQUEST_STATUS.OBSERVED,
      requester_id: "user-1",
    });
    const observedRexan = makeRequest({
      request_type: REQUEST_TYPE.ADVANCE_SETTLEMENT,
      status: REQUEST_STATUS.OBSERVED,
      requester_id: "user-1",
    });
    ...
    expect(
      canManageRequestDocuments(
        ROLE_CODE.GIOF_GESTOR,
        REQUEST_STATUS.OBSERVED,
        { ...observed, giof_work: { canEdit: true } } as PaymentRequest,
        "giof-1",
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      canManageRequestDocuments(
        ROLE_CODE.GIOF_MANAGER,
        REQUEST_STATUS.PAID,
        { ...draft, giof_work: { canEdit: true } } as PaymentRequest,
        "manager-1",
      ),
    ).toBe(true);
    expect(
    ...
    ).toContain("borrador u observación");
    expect(
      getRequestDocumentPermissionMessage(
        ROLE_CODE.SOLICITANTE_EPE,
        REQUEST_STATUS.DRAFT,
    ...
    ).toContain("solicitante titular");
  });

  it("bloquea solo cuando el techo de unidad orgánica excede", () => {
    expect(isBudgetPreviewBlocking(makePreview({ lineWarning: true }))).toBe(
      false,
    );
    expect(
      isBudgetPreviewBlocking(
  ...
  it("muestra fecha y hora en formato de lista", () => {
    ...
    expect(formatRequestDateTime("2026-05-07T15:30:00.000Z")).toMatch(
      /\d{1,2}:\d{2}/,
    );
  ...
  it("expone cards y opciones de orden para la bandeja de solicitudes", () => {
    ...
    ).not.toContain(REQUEST_STATUS.CLOSED);
  });
  ...
  it("mapea las colas operativas GIOF a filtros de la bandeja de revisión", () => {
    ...
    expect(
      getRequestReviewQueueFilter(REQUEST_REVIEW_QUEUE.PENDING_LEVEL_1),
    ).toMatchObject({
      status: REQUEST_STATUS.SUBMITTED,
      sort: REQUEST_LIST_SORT.REVIEW_PRIORITY,
    });
    expect(
      getRequestReviewQueueFilter(REQUEST_REVIEW_QUEUE.PENDING_LEVEL_2),
    ).toMatchObject({
      status: REQUEST_STATUS.IN_VALIDATION,
      sort: REQUEST_LIST_SORT.OLDEST_FIRST,
    });
    expect(
      getRequestReviewQueueFilter(REQUEST_REVIEW_QUEUE.OBSERVED_RETURNED),
    ).toMatchObject({
      status: REQUEST_STATUS.OBSERVED,
      sort: REQUEST_LIST_SORT.NEWEST_FIRST,
    });
    expect(
      getRequestReviewQueueFilter(REQUEST_REVIEW_QUEUE.BLOCKED)
        .unsupportedReason,
    ).toContain("Funcionalidad en preparación");
  });

  it("parsea filtros compartibles desde query params de revisión", () => {
    expect(parseRequestReviewQueue("pending-level-1")).toBe(
      REQUEST_REVIEW_QUEUE.PENDING_LEVEL_1,
    );
    expect(parseRequestReviewQueue("unknown")).toBeUndefined();
    expect(parseRequestStatusFilter("IN_VALIDATION")).toBe(
      REQUEST_STATUS.IN_VALIDATION,
    );
    expect(parseRequestStatusFilter("unknown")).toBeUndefined();
    expect(parseRequestListSort("OLDEST_FIRST")).toBe(
      REQUEST_LIST_SORT.OLDEST_FIRST,
    );
    expect(parseRequestListSort("unknown")).toBe(
      REQUEST_LIST_SORT.NEWEST_FIRST,
    );
  });
  ...
  it("construye el avance de solicitud sin completar la ruta de aprobación para observadas", () => {
    const steps = getRequestStatusStepperItems(REQUEST_STATUS.OBSERVED, [
      ...
      makeHistoryItem({
        ...
        to_status: REQUEST_STATUS.OBSERVED,
    ...
    expect(
      steps.find((step) => step.status === REQUEST_STATUS.OBSERVED)?.state,
    ).toBe("current");
    expect(
      steps.find((step) => step.status === REQUEST_STATUS.APPROVED)?.state,
    ).toBe("pending");
  });
  ...
  it("usa una rama terminal para rechazadas sin mostrar aprobación como completada", () => {
    ...
    expect(
      steps.find((step) => step.status === REQUEST_STATUS.REJECTED)?.isBranch,
    ).toBe(true);
    expect(steps.some((step) => step.status === REQUEST_STATUS.APPROVED)).toBe(
      false,
    );
  });
  ...
  it("incluye validación solo cuando el estado aparece en el flujo", () => {
    const steps = getRequestStatusStepperItems(REQUEST_STATUS.IN_VALIDATION, [
      ...
      makeHistoryItem({
        from_status: REQUEST_STATUS.SUBMITTED,
        to_status: REQUEST_STATUS.IN_VALIDATION,
      }),
    ]);
  ...
  it("usa el vocabulario de detalle para no confundir revisión, aprobación y pago", () => {
    expect(getRequestStatusLabel(REQUEST_STATUS.SUBMITTED)).toBe(
      "Enviada a revisión",
    );
    expect(
      getRequestStatusLabel(
    ...
    ).toBe("Aprobada · pendiente de pago");
  ...
  it("muestra aprobaciones REXAN resueltas con contexto de rendición y EXCESS como pago pendiente", () => {
    ...
    expect(
      getRequestStatusLabel(REQUEST_STATUS.APPROVED, exactSettlement),
    ).toBe("Rendición aprobada");
    expect(
      getRequestStatusLabel(REQUEST_STATUS.APPROVED, returnSettlement),
    ).toBe("Rendición aprobada");
    expect(
      getRequestStatusLabel(REQUEST_STATUS.APPROVED, excessSettlement),
    ).toBe("Rendición aprobada");

    const exactSteps = getRequestStatusStepperItems(
      REQUEST_STATUS.APPROVED,
      [],
      {},
      exactSettlement,
    );
    const returnSteps = getRequestStatusStepperItems(
      REQUEST_STATUS.APPROVED,
      [],
      {},
      returnSettlement,
    );
    const excessApprovedSteps = getRequestStatusStepperItems(
      REQUEST_STATUS.APPROVED,
      [],
      {},
      excessSettlement,
    );
    const excessPaidSteps = getRequestStatusStepperItems(
      REQUEST_STATUS.PAID,
      [],
      {},
      paidExcessSettlement,
    );
    const normalApprovedSteps = getRequestStatusStepperItems(
      REQUEST_STATUS.APPROVED,
      [],
      {},
      normalApproved,
    ...
    expect(
      exactSteps.find((step) => step.status === REQUEST_STATUS.APPROVED)?.label,
    ).toBe("Rendición aprobada");
    ...
    expect(
      excessApprovedSteps.find(
        (step) => step.status === REQUEST_STATUS.APPROVED,
      )?.label,
    ).toBe("Rendición aprobada");
    expect(
      excessApprovedSteps.find((step) => step.status === REQUEST_STATUS.PAID)
        ?.state,
    ).toBe("pending");
    expect(
      excessPaidSteps.find((step) => step.status === REQUEST_STATUS.PAID)
        ?.state,
    ).toBe("current");
    expect(normalApprovedSteps.map((step) => step.status)).toContain(
      REQUEST_STATUS.PAID,
    );
  });
  ...
  it("mantiene CLOSED solo como compatibilidad cuando aparece en estado o historial", () => {
    expect(
      getRequestStatusStepperItems(REQUEST_STATUS.APPROVED).map(
        (step) => step.status,
      ),
    ).not.toContain(REQUEST_STATUS.CLOSED);
    expect(
      getRequestStatusStepperItems(REQUEST_STATUS.PAID).map(
        (step) => step.status,
      ),
    ).toEqual([
      REQUEST_STATUS.DRAFT,
    ...
    expect(
      getRequestStatusStepperItems(REQUEST_STATUS.CLOSED, [
        makeHistoryItem({
          from_status: REQUEST_STATUS.PAID,
          to_status: REQUEST_STATUS.CLOSED,
        }),
  ...
  it("ordena por prioridad de revisión con enviadas FIFO primero", () => {
    ...
    const newestSubmitted = makeRequest({
      id: "new-submitted",
      status: REQUEST_STATUS.SUBMITTED,
      submitted_at: "2026-05-07T10:00:00.000Z",
    });
    const oldestSubmitted = makeRequest({
      id: "old-submitted",
      status: REQUEST_STATUS.SUBMITTED,
    ...
    expect(
      sortRequestsForList(
        [draft, newestSubmitted, oldestSubmitted],
        REQUEST_LIST_SORT.REVIEW_PRIORITY,
      ).map((request) => request.id),
    ).toEqual(["old-submitted", "new-submitted", "draft"]);
  });

  it("ordena por antigüedad o recencia usando envío y creación", () => {
    const oldest = makeRequest({
      id: "oldest",
      created_at: "2026-05-01T10:00:00.000Z",
    });
    ...
    expect(
      sortRequestsForList([newest, oldest], REQUEST_LIST_SORT.OLDEST_FIRST).map(
        (request) => request.id,
      ),
    ).toEqual(["oldest", "newest"]);
    expect(
      sortRequestsForList([oldest, newest], REQUEST_LIST_SORT.NEWEST_FIRST).map(
        (request) => request.id,
      ),
  ...
  it("expone ayudas dinámicas por tipo de documento del beneficiario", () => {
    expect(
      getBeneficiaryDocumentPlaceholder(BENEFICIARY_DOCUMENT_TYPE.DNI),
    ).toBe("8 dígitos");
    expect(
      getBeneficiaryDocumentPlaceholder(BENEFICIARY_DOCUMENT_TYPE.RUC),
    ).toBe("11 dígitos");
    expect(getBeneficiaryDocumentHelp(BENEFICIARY_DOCUMENT_TYPE.CE)).toContain(
      "6 a 12",
    );
    expect(getBeneficiaryDocumentMaxLength(BENEFICIARY_DOCUMENT_TYPE.DNI)).toBe(
      8,
    );
    expect(getBeneficiaryDocumentInputMode(BENEFICIARY_DOCUMENT_TYPE.RUC)).toBe(
      "numeric",
    );
    expect(getBeneficiaryDocumentInputMode(BENEFICIARY_DOCUMENT_TYPE.CE)).toBe(
      "text",
    );
  });

  it("obtiene etiqueta y contraparte para cola de pagos", () => {
    expect(getPaymentQueueStatusLabel(REQUEST_STATUS.APPROVED)).toBe(
      "Pendiente de pago",
    );
    expect(getPaymentQueueStatusLabel(REQUEST_STATUS.PAID)).toBe(
      "Pago registrado",
    );
    expect(
      getPaymentRequestParty(
        makeRequest({ registered_party_name: "Proveedor SAC" }),
      ),
    ).toBe("Proveedor SAC");
    expect(
      getRegisteredPartyDisplay(
        makeRequest({ supplier_name: "Proveedor Base SAC" }),
      ),
    ).toBe("Proveedor Base SAC");
    expect(
      getRegisteredPartyDisplay(
        makeRequest({ beneficiary_name: "Beneficiario" }),
      ),
    ).toBe("Beneficiario");
    expect(
      getRegisteredPartyDocumentLabel(
        makeRequest({
          registered_party_document_type: BENEFICIARY_DOCUMENT_TYPE.RUC,
          registered_party_document_number: "20123456789",
        }),
      ),
    ).toBe("RUC 20123456789");
    expect(
      getRegisteredPartyDocumentLabel(
        makeRequest({ requester_name: "Ana Paredes" }),
      ),
    ).toBe("—");
    expect(
      getRequestPayableAmount(
  ...
  it("sanitiza campos numéricos de pago con longitud máxima", () => {
    expect(sanitizeDigits("001-abc-234567890123456789012345678901", 30)).toBe(
      "001234567890123456789012345678",
    );
    expect(sanitizeDigits("002-abc", 20)).toBe("002");
  });

  it("oculta nombres técnicos en mensajes presupuestales", () => {
    expect(sanitizeBudgetMessage("budget_ceiling no configurado")).toBe(
      "límite presupuestal no configurado",
    );
  });

  it("habilita revisión a gestor y manager en solicitudes enviadas", () => {
    expect(
      canReviewRequest(ROLE_CODE.GIOF_GESTOR, REQUEST_STATUS.SUBMITTED),
    ).toBe(true);
    expect(
      canReviewRequest(ROLE_CODE.GIOF_MANAGER, REQUEST_STATUS.SUBMITTED),
    ).toBe(true);
    expect(
      canReviewRequest(ROLE_CODE.ADMIN_SISTEMA, REQUEST_STATUS.SUBMITTED),
    ).toBe(false);
    expect(
      canReviewRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.SUBMITTED),
    ).toBe(false);
    expect(
      canReviewRequest(ROLE_CODE.GIOF_GESTOR, REQUEST_STATUS.OBSERVED),
    ).toBe(false);
  });

  it("habilita edición de borrador para cualquier rol autenticado", () => {
    expect(
      canEditDraftRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.DRAFT),
    ).toBe(true);
    expect(
      canEditDraftRequest(ROLE_CODE.GIOF_GESTOR, REQUEST_STATUS.DRAFT),
    ).toBe(true);
    expect(
      canEditDraftRequest(ROLE_CODE.AUDITOR_DIRECCION, REQUEST_STATUS.DRAFT),
    ).toBe(true);
    expect(
      canEditDraftRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.SUBMITTED),
    ).toBe(false);
  });
  ...
  it("habilita corrección para cualquier rol autenticado en solicitudes observadas", () => {
    ...
    expect(
      canCorrectObservedRequest(ROLE_CODE.GIOF_GESTOR, REQUEST_STATUS.OBSERVED),
    ).toBe(true);
    expect(
      canCorrectObservedRequest(
        ROLE_CODE.AUDITOR_DIRECCION,
  ...
  it("consolida edición para borradores y solicitudes observadas", () => {
    expect(
      canEditRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.DRAFT),
    ).toBe(true);
    expect(
      canEditRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.OBSERVED),
    ).toBe(true);
    expect(
      canEditRequest(ROLE_CODE.SOLICITANTE_EPE, REQUEST_STATUS.SUBMITTED),
    ).toBe(false);
    expect(canEditRequest(ROLE_CODE.GIOF_GESTOR, REQUEST_STATUS.OBSERVED)).toBe(
      true,
    );
  });
