...
const REQUESTS_RATE_LIMIT_MESSAGE =
...
type RequestHistoryDateMode =
  (typeof REQUEST_HISTORY_DATE_MODE)[keyof typeof REQUEST_HISTORY_DATE_MODE];
...
function getServerSearchValue(value: string): string | undefined {
  ...
  return trimmedValue.length >= REQUEST_SEARCH_MIN_LENGTH
    ? trimmedValue
    : undefined;
}

function getRequestsListErrorMessage(
  error: Error,
  useSafeReviewMessage = false,
): string {
  if (error instanceof ApiRequestError && error.status === 429)
    return REQUESTS_RATE_LIMIT_MESSAGE;
  if (/throttlerexception|too many requests/i.test(error.message))
    return REQUESTS_RATE_LIMIT_MESSAGE;
  if (useSafeReviewMessage)
    return "No se pudieron cargar las solicitudes para revisión. Reintenta en unos segundos.";
...
function getRequestReviewExportErrorMessage(error: unknown): string {
  if (isAbortError(error)) return "La descarga del Excel fue cancelada.";
  if (error instanceof ApiRequestError) {
    if (error.status === 400)
      return "Revisa los filtros activos antes de exportar el Excel.";
    if (error.status === 401)
      return "Tu sesión no está disponible. Vuelve a iniciar sesión para descargar el Excel.";
    if (error.status === 403)
      return "No tienes permisos para exportar la Bandeja de Revisión.";
    if (error.status === 422) {
      const { count, max } = error.body;
      if (
        error.body.code === "REQUEST_REVIEW_EXPORT_ROW_LIMIT_EXCEEDED" &&
        typeof count === "number" &&
        ...
        return `El Excel tendría ${formatter.format(count)} filas y el máximo permitido es ${formatter.format(max)}. Ajusta los filtros activos.`;
      }
      return "No se pudo generar el Excel con los filtros activos. Ajusta los filtros e inténtalo nuevamente.";
    }
    if (error.status === 429)
      return "Hay muchas exportaciones seguidas. Espera unos segundos e inténtalo nuevamente.";
  }
  return "No se pudo descargar el Excel. Inténtalo nuevamente.";
}
...
export function RequestsPage() {
  ...
  const serverSearch = searchParams.has("search")
    ? getServerSearchValue(debouncedSearch)
    : undefined;
  const [status, setStatus] = useState<RequestStatus | undefined>(queryStatus);
  const [activeQueue, setActiveQueue] = useState<
    RequestReviewQueue | undefined
  >(queryQueue ?? getRequestReviewQueueForStatus(queryStatus));
  const [sort, setSort] = useState<RequestListSort>(
    parseRequestListSort(searchParams.get("sort")),
  );
  ...
  const [dateMode, setDateMode] = useState<RequestHistoryDateMode>(() => {
    ...
    return initialDateFrom && initialDateFrom === initialDateTo
      ? REQUEST_HISTORY_DATE_MODE.EXACT
      : REQUEST_HISTORY_DATE_MODE.RANGE;
  });
  const [dateField, setDateField] = useState<RequestListDateField>(
    (searchParams.get("date_field") as RequestListDateField | null) ??
      REQUEST_LIST_DATE_FIELD.UPDATED_AT,
  );
  const [requestType, setRequestType] = useState<RequestType | undefined>(
    (searchParams.get("request_type") as RequestType | null) ?? undefined,
  );
  ...
  const [planningLineId, setPlanningLineId] = useState(
    searchParams.get("budget_planning_line_id") ?? "",
  );
  const [hasDocuments, setHasDocuments] = useState<string>(
    searchParams.get("has_documents") ?? "ALL",
  );
  const [driveSyncStatus, setDriveSyncStatus] = useState<
    DriveSyncStatus | undefined
  >(
    (searchParams.get("drive_sync_status") as DriveSyncStatus | null) ??
      undefined,
  );
  ...
  const reviewExportControllerRef = useRef<AbortController | null>(null);
  const reviewExportInFlightRef = useRef(false);

  useEffect(() => {
    const nextStatus = parseRequestStatusFilter(searchParams.get("status"));
    const nextQueue =
      parseRequestReviewQueue(searchParams.get("queue")) ??
      getRequestReviewQueueForStatus(nextStatus);
    ...
    setDateMode(
      nextDateFrom && nextDateFrom === nextDateTo
        ? REQUEST_HISTORY_DATE_MODE.EXACT
        : REQUEST_HISTORY_DATE_MODE.RANGE,
    );
    setDateField(
  ...
  useEffect(() => () => reviewExportControllerRef.current?.abort(), []);
  ...
  const canUseHistory = hasRoleCapability(
    roleCode,
    ROLE_CAPABILITY.REQUEST_HISTORY,
  );
  const rawScope = searchParams.get("scope");
  const requestScope: RequestScope =
    rawScope === REQUEST_SCOPE.HISTORY && (canUseHistory || isRolePending)
      ? REQUEST_SCOPE.HISTORY
      : rawScope === REQUEST_SCOPE.REVIEW &&
          (isGiofOperational || isRolePending)
  ...
  const workAssigneeId =
    workScope === GIOF_WORK_SCOPE.ASSIGNEE
      ...
      : undefined;
  const activeQueueFilter = activeQueue
    ? getRequestReviewQueueFilter(activeQueue)
    : undefined;
  const isUnsupportedQueue = Boolean(activeQueueFilter?.unsupportedReason);
  const defaultReviewStatuses =
    isReviewInbox && !status && !activeQueue && !isExplicitAllStatuses
      ? [...ACTIVE_REVIEW_STATUSES]
      : undefined;
  ...
  const hasUnsupportedReviewUrl =
    parsedReviewUrl.invalidKeys.length > 0 ||
  ...
  const legacyResource = useRequests(
    {
      ...
      budget_planning_line_id: isHistory
        ? planningLineId.trim() || undefined
        : undefined,
      has_documents:
        isHistory && hasDocuments !== "ALL"
          ? hasDocuments === "true"
          : undefined,
  ...
  const requests =
    isReviewInbox && !isRolePending
      ? reviewResource.requests
      : isReviewInbox
        ? []
        : legacyResource.requests;
  const total =
    isReviewInbox && !isRolePending
      ? reviewResource.total
      : isReviewInbox
        ? 0
        : legacyResource.total;
  const isLoading = isReviewInbox
    ? isRolePending || reviewResource.isLoading
    : legacyResource.isLoading;
  const isRefreshing = isReviewInbox
    ? reviewResource.isRefreshing
    : legacyResource.isRefreshing;
  const error = isReviewInbox ? reviewResource.error : legacyResource.error;
  const refetch = isReviewInbox
    ? reviewResource.refetch
    : legacyResource.refetch;
  const displayedRequests = isReviewInbox
    ? requests
  ...
  function setStatusFilter(value: RequestStatus | undefined) {
    const nextQueue = isGiofReviewInbox
      ? getRequestReviewQueueForStatus(value)
      : undefined;
    setActiveQueue(nextQueue);
    setStatus(value);
    replaceQuery({
      status: value ?? ALL_STATUSES_FILTER,
      queue: nextQueue,
      page: 1,
    });
  }
  ...
  function updateReviewFilters(patch: Partial<ReviewFilters>) {
    setSelectedAssignmentIds([]);
    const nextParams = updateRequestReviewUrl(
  ...
  async function exportReviewExcel() {
    if (
      reviewExportInFlightRef.current ||
      !isReviewInbox ||
      !isGiofOperational ||
      hasUnsupportedReviewUrl
    )
    ...
    reviewExportControllerRef.current = controller;
    reviewExportInFlightRef.current = true;
    setIsExportingReview(true);

    try {
      const download = await downloadRequestReviewExport(
        reviewFilters,
        controller.signal,
      );
      if (controller.signal.aborted)
        throw new DOMException("Aborted", "AbortError");
      saveRequestReviewExport(download);
      toast.success("Excel de Bandeja de Revisión descargado correctamente.");
    } catch (exportError) {
      toast.error(getRequestReviewExportErrorMessage(exportError));
    } finally {
      if (reviewExportControllerRef.current === controller) {
        reviewExportControllerRef.current = null;
        reviewExportInFlightRef.current = false;
        setIsExportingReview(false);
      }
    }
  }

  function cancelReviewExport() {
    reviewExportControllerRef.current?.abort();
  }
  ...
  function setHistoryDateMode(nextMode: RequestHistoryDateMode) {
    ...
    if (nextMode === REQUEST_HISTORY_DATE_MODE.EXACT) {
      ...
      setDateTo(exactDate);
      replaceQuery({
        date_from: exactDate || undefined,
        date_to: exactDate || undefined,
        page: 1,
      });
    }
  }
  ...
  function getStatusCount(cardStatus: RequestStatus) {
    return summaryRequests.filter((request) => request.status === cardStatus)
      .length;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1
            ...
            data-testid="requests-page-title"
          >
            {isHistory
          ...
          <p className="text-muted-foreground">
            {isHistory
              ? "Consulta solicitudes históricas con filtros avanzados."
              : isReviewInbox
                ? "Abre una solicitud enviada para observar, aprobar o rechazar."
                : "Consulta y crea solicitudes de pago."}
          </p>
        </div>
        {requestScope === REQUEST_SCOPE.MINE && (
          <Button
            onClick={() => router.push(ROUTES.REQUESTS_NEW)}
      ...
      <div className="flex flex-wrap gap-2" data-testid="requests-scope-tabs">
        <Button
          variant={requestScope === "mine" ? "default" : "outline"}
        ...
        {isGiofOperational && (
          <Button
            variant={requestScope === "review" ? "default" : "outline"}
        ...
        {canUseHistory && (
          <Button
            variant={requestScope === "history" ? "default" : "outline"}
            onClick={() => setScopeFilter("history")}
          >
            Historial
          </Button>
        )}
      ...
      {requestScope !== REQUEST_SCOPE.MINE && !isReviewInbox && (
        <div
          className={cn(
            "grid gap-3 sm:grid-cols-2",
            isGiofReviewInbox ? "lg:grid-cols-3" : "lg:grid-cols-5",
          )}
          data-testid="requests-status-summary"
        >
          {isGiofReviewInbox
            ? REQUEST_REVIEW_QUEUE_CARDS.map((card) => {
                ...
                return (
                  ...
                  >
                    <p className="text-2xl font-bold text-foreground">
                      {getRequestReviewQueueCount(summaryRequests, card.value)}
                    </p>
            ...
            : REQUEST_STATUS_SUMMARY_CARDS.map((card) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setStatusFilter(isActive ? undefined : card.value)
                    }
                    className={cn(
                      "rounded-lg border bg-card p-4 text-left shadow-sm transition-all hover:shadow-md",
                      isActive
      ...
      {activeQueueFilter?.unsupportedReason && (
        <p
          ...
          data-testid="requests-review-queue-note"
        >
          {activeQueueFilter.unsupportedReason}
        </p>
      )}

      {isGiofReviewInbox && !hasUnsupportedReviewUrl && (
        <GiofClaimableWorkPanel
          pool={GIOF_WORK_POOL.REQUEST}
          refetchPoolQueue={(options) => reviewResource.refetch(options)}
        />
      )}
        ...
        <CardHeader className="gap-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <CardDescription>
                {isHistory
                  ? "Filtra por fechas, responsable, documentos y Drive."
                  : isReviewInbox
                    ? "Abre una solicitud enviada para observar, aprobar o rechazar."
            ...
            {isReviewInbox && (isGiofOperational || isRolePending) && (
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => void exportReviewExcel()}
                  disabled={
                    isRolePending ||
                    hasUnsupportedReviewUrl ||
                    isExportingReview
                  }
                  aria-busy={isExportingReview}
                  aria-describedby="request-review-export-state"
                >
                  {isExportingReview ? (
                    <Loader2
                  ...
                  {isExportingReview ? "Exportando Excel..." : "Exportar Excel"}
                </Button>
                {isExportingReview && (
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={cancelReviewExport}
                  >
                    Cancelar descarga
                  </Button>
                )}
                <span
                  id="request-review-export-state"
                  className={
                    isExportingReview
                      ? "text-sm text-muted-foreground"
                      : "sr-only"
                  }
                  role={isExportingReview ? "status" : undefined}
                  aria-live="polite"
                >
                  {isExportingReview
                    ? "Preparando el Excel. La cancelación es best-effort."
                    : isRolePending
                      ? "Esperando la información del rol para habilitar la exportación."
                      : hasUnsupportedReviewUrl
                        ? "Restablece los filtros inválidos antes de exportar."
                        : "Exporta todo el universo que coincide con los filtros activos."}
                </span>
              </div>
            )}
            {!isReviewInbox && (
              <div className="flex w-full flex-col gap-2 lg:max-w-3xl">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                  {requestScope !== REQUEST_SCOPE.MINE && (
                    <Select
                      value={
                        isExplicitAllStatuses
                          ? ALL_STATUSES_FILTER
                          : (status ?? ALL_STATUSES_FILTER)
                      }
                      onValueChange={(value) =>
                        setStatusFilter(
                          value === ALL_STATUSES_FILTER
                            ? undefined
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={ALL_STATUSES_FILTER}>
                          Ver todo
                        </SelectItem>
                  ...
                  {!isReviewInbox && (
                    <Select
                      value={sort}
                      onValueChange={(value) =>
                        setSortOption(value as RequestListSort)
                      }
                    >
                      <SelectTrigger
                        className="sm:w-52"
                        data-testid="requests-sort-control"
                      >
                  ...
                  {isHistory && activeHistoryFiltersCount > 0 && (
                    <Button
                      ...
                      data-testid="requests-clear-history-filters"
                    >
                      <X className="h-4 w-4" />
          ...
          {isHistory && (
            <div
              ...
              data-testid="requests-history-filters"
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                <div className="space-y-2 lg:w-52">
                  <label
                    className="text-sm font-medium"
                    htmlFor="history-date-mode"
                  >
                  ...
                  <Select
                    value={dateMode}
                    onValueChange={(value) =>
                      setHistoryDateMode(value as RequestHistoryDateMode)
                    }
                  >
                    <SelectTrigger id="history-date-mode">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value={REQUEST_HISTORY_DATE_MODE.EXACT}>
                        Fecha exacta
                      </SelectItem>
                      <SelectItem value={REQUEST_HISTORY_DATE_MODE.RANGE}>
                        Rango de fechas
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                {dateMode === REQUEST_HISTORY_DATE_MODE.EXACT ? (
                  <div className="space-y-2 lg:w-48">
                    <label
                      className="text-sm font-medium"
                      htmlFor="history-exact-date"
                    >
                      Fecha
                    </label>
                    <Input
                      id="history-exact-date"
                      type="date"
                      value={dateFrom}
                      onChange={(event) =>
                        setHistoryExactDateFilter(event.target.value)
                      }
                ...
                <div className="space-y-2 lg:w-60">
                  ...
                  <Select
                    value={dateField}
                    onValueChange={(value) => {
                      setDateField(value as RequestListDateField);
                      setAdvancedFilter("date_field", value);
                    }}
                  >
                    <SelectTrigger id="history-date-field">
                      <SelectValue placeholder="Campo fecha" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value={REQUEST_LIST_DATE_FIELD.UPDATED_AT}>
                        Última actualización
                      </SelectItem>
                      <SelectItem value={REQUEST_LIST_DATE_FIELD.CREATED_AT}>
                        Creación
                      </SelectItem>
                      <SelectItem value={REQUEST_LIST_DATE_FIELD.SUBMITTED_AT}>
                        Envío
                      </SelectItem>
                      <SelectItem value={REQUEST_LIST_DATE_FIELD.APPROVED_AT}>
                        Aprobación
                      </SelectItem>
                      <SelectItem value={REQUEST_LIST_DATE_FIELD.PAID_AT}>
                        Pago
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2 lg:w-56">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      const next =
                        value === "ALL" ? undefined : (value as RequestType);
                      setRequestType(next);
                      setAdvancedFilter("request_type", next);
                    }}
                  >
                    <SelectTrigger id="history-request-type">
                      <SelectValue placeholder="Tipo" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="ALL">Todos los tipos</SelectItem>
                      <SelectItem value={REQUEST_TYPE.ADVANCE}>
                        Anticipo
                      </SelectItem>
                      <SelectItem value={REQUEST_TYPE.REIMBURSEMENT}>
                        Reembolso
                      </SelectItem>
                      <SelectItem value={REQUEST_TYPE.SUPPLIER_PAYMENT}>
                        Pago a proveedor
                      </SelectItem>
                      <SelectItem value={REQUEST_TYPE.ADVANCE_SETTLEMENT}>
                        Rendición
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              <p className="text-xs text-muted-foreground">
                Fecha exacta envía el mismo día como inicio y fin; el backend
                usa fin inclusivo del día completo.
              </p>
              <div className="flex flex-wrap items-center gap-2">
                {activeHistoryFiltersCount > 0 && (
                  <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                    {activeHistoryFiltersCount} filtros activos
                  </span>
                )}
                <Button
                  type="button"
              ...
              {showAdvancedFilters && (
                <div className="grid gap-3 border-t pt-4 sm:grid-cols-2 xl:grid-cols-5">
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor="history-requester-id"
                    >
                      Solicitante (UUID)
                    </label>
                    <Input
                      id="history-requester-id"
                      value={requesterId}
                      onChange={(event) => {
                      ...
                      placeholder="Pegar UUID"
                    />
                  </div>
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor="history-org-unit-id"
                    >
                      Unidad organizacional (UUID)
                    </label>
                    <Input
                      id="history-org-unit-id"
                      value={orgUnitId}
                      onChange={(event) => {
                  ...
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor="history-planning-line-id"
                    >
                      Línea POA (UUID)
                    </label>
                    <Input
                      id="history-planning-line-id"
                      value={planningLineId}
                      onChange={(event) => {
                        setPlanningLineId(event.target.value);
                        setAdvancedFilter(
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger id="history-has-documents">
                  ...
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor="history-drive-status"
                    >
                      Estado Drive
                    </label>
                    <Select
                      value={driveSyncStatus ?? "ALL"}
                      onValueChange={(value) => {
                        const next =
                          value === "ALL"
                            ? undefined
                            : (value as DriveSyncStatus);
                        setDriveSyncStatus(next);
                        setAdvancedFilter("drive_sync_status", next);
                      }}
                    >
                      <SelectTrigger id="history-drive-status">
                        <SelectValue placeholder="Estado Drive" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="ALL">Todos Drive</SelectItem>
                        {Object.values(DRIVE_SYNC_STATUS).map((value) => (
          ...
          {hasUnsupportedReviewUrl ? null : error ? (
            ...
            >
              <p className="text-sm text-destructive">
                {getRequestsListErrorMessage(error, isReviewInbox)}
              </p>
              <Button
                size="sm"
                variant="outline"
