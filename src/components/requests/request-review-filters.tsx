...
const REQUEST_STATUS_OPTIONS = REQUEST_REVIEW_SELECTOR_STATUSES.map(
  (value) => ({
    value,
...
function getActiveChips(filters: ReviewFilters): ActiveFilterChip[] {
  const chips: ActiveFilterChip[] = [];
  if (filters.work_scope && filters.work_scope !== GIOF_WORK_SCOPE.ALL) {
  ...
  if (filters.request_type)
    chips.push({
      key: "request-type",
      label: `Tipo: ${REQUEST_TYPE_LABEL[filters.request_type]}`,
      patch: { request_type: undefined },
    });
  if (filters.status)
    chips.push({
      key: "status",
      label: `Estado: ${getReviewStatusLabel(filters.status)}`,
      patch: { status: undefined },
    });
  if (filters.submitted_from && filters.submitted_to)
    chips.push({
      key: "submitted",
      label: `Enviada: ${filters.submitted_from} – ${filters.submitted_to}`,
      patch: { submitted_from: undefined, submitted_to: undefined },
    });
  if (filters.assigned_from && filters.assigned_to)
    chips.push({
      key: "assigned",
      label: `Asignada: ${filters.assigned_from} – ${filters.assigned_to}`,
      patch: { assigned_from: undefined, assigned_to: undefined },
    });
  if (filters.currency) {
  ...
  if (filters.org_unit_id)
    chips.push({
      key: "org-unit",
      label: "Unidad organizacional seleccionada",
      patch: { org_unit_id: undefined },
    });
  return chips;
}

export function RequestReviewFilters({
  filters,
  isManager,
  total,
  summary,
  ...
  function applyAdvancedFilters(): void {
    if (
      ...
      setValidationError(
        "Revisa los intervalos y montos. Cada intervalo requiere un inicio anterior al fin y los montos requieren moneda y dos decimales.",
      );
  ...
  const currencySummary = summary
    ? Object.entries(summary.requested_amount_by_currency).filter(
        (entry): entry is [string, string] => entry[1] !== undefined,
      )
    : [];
  const statusSummary = summary
    ? Object.entries(summary.status_counts).filter(
        (entry): entry is [RequestStatus, number] => entry[1] !== undefined,
      )
    : [];
  ...
  return (
    <section
      ...
      aria-label="Filtros de revisión de solicitudes"
    >
      <GiofWorkScopeFilter
        value={filters.work_scope ?? GIOF_WORK_SCOPE.ALL}
        assigneeId={filters.assignee_id}
        isManager={isManager}
        onChange={(workScope, assigneeId) =>
          onChange({
            work_scope: workScope,
            assignee_id:
              workScope === GIOF_WORK_SCOPE.ASSIGNEE ? assigneeId : undefined,
          })
        }
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_14rem_14rem]">
        ...
        >
          <div className="relative min-w-0 flex-1">
            <label className="sr-only" htmlFor="request-review-search">
              Buscar solicitudes
            </label>
            <Search
              aria-hidden="true"
            ...
            />
            <Input
              id="request-review-search"
              value={search}
              maxLength={200}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Código, concepto, beneficiario o línea POA"
              className="pl-9"
            />
          </div>
        ...
        <div>
          <label className="sr-only" htmlFor="request-review-type">
            Tipo de solicitud
          </label>
          <Select
            value={filters.request_type ?? QUEUE_FILTER_ALL_VALUE}
            onValueChange={(value) =>
              onChange({
                request_type:
                  value === QUEUE_FILTER_ALL_VALUE
                    ? undefined
                    : (value as RequestType),
              })
            }
          >
            <SelectTrigger id="request-review-type">
        ...
        <div>
          <label className="sr-only" htmlFor="request-review-status">
            Estado de solicitud
          </label>
          <Select
            value={filters.status ?? QUEUE_FILTER_ALL_VALUE}
            onValueChange={(value) =>
              onChange({
                status:
                  value === QUEUE_FILTER_ALL_VALUE
      ...
      <Button
        ...
        aria-controls="request-review-advanced"
      ...
      {advancedOpen && (
        <div id="request-review-advanced" className="space-y-4 border-t pt-4">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="space-y-1">
              <label
                ...
                htmlFor="review-submitted-from"
              >
                Enviada desde
              </label>
              <Input
                ref={firstAdvancedInputRef}
                id="review-submitted-from"
                type="datetime-local"
                value={submittedFrom}
            ...
            <div className="space-y-1">
              <label
                className="text-sm font-medium"
                htmlFor="review-submitted-to"
              >
                Enviada hasta
              </label>
              <Input
                id="review-submitted-to"
                type="datetime-local"
                value={submittedTo}
            ...
            <div className="space-y-1">
              <label
                className="text-sm font-medium"
                htmlFor="review-assigned-from"
              >
                Asignada desde
              </label>
              <Input
                id="review-assigned-from"
                type="datetime-local"
                value={assignedFrom}
            ...
            <div className="space-y-1">
              <label
                className="text-sm font-medium"
                htmlFor="review-assigned-to"
              >
                Asignada hasta
              </label>
              <Input
                id="review-assigned-to"
                type="datetime-local"
                value={assignedTo}
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="space-y-1">
              <label className="text-sm font-medium" htmlFor="review-currency">
                Moneda solicitada
              </label>
              <Select
                value={currency || QUEUE_FILTER_ALL_VALUE}
                onValueChange={(value) =>
                  setCurrency(value === QUEUE_FILTER_ALL_VALUE ? "" : value)
                }
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value={REQUEST_CURRENCY.PEN}>PEN</SelectItem>
                  <SelectItem value={REQUEST_CURRENCY.USD}>USD</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              <label
                className="text-sm font-medium"
                htmlFor="review-amount-min"
              >
                Monto mínimo solicitado
              </label>
              <Input
                id="review-amount-min"
                inputMode="decimal"
                value={amountMin}
            ...
            <div className="space-y-1">
              <label
                className="text-sm font-medium"
                htmlFor="review-amount-max"
              >
                Monto máximo solicitado
              </label>
              <Input
                id="review-amount-max"
                inputMode="decimal"
                value={amountMax}
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium" htmlFor="review-org-unit">
                Unidad organizacional
              </label>
              <Select
                value={orgUnitId || QUEUE_FILTER_ALL_VALUE}
                onValueChange={(value) =>
                  setOrgUnitId(value === QUEUE_FILTER_ALL_VALUE ? "" : value)
                }
                disabled={orgUnitsLoading || Boolean(orgUnitsError)}
              >
                <SelectTrigger id="review-org-unit">
                  <SelectValue
                    placeholder={
                      orgUnitsLoading
                        ? "Cargando unidades"
          ...
          {orgUnitsError && (
            <p className="text-sm text-destructive">
              No se pudieron cargar las unidades autorizadas.
            </p>
          )}
          {validationError && (
      ...
      >
        {liveMessage}
        {summary ? " en la lista y el resumen" : ""}.
      </p>
      {summary && (
        <div
          className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"
          aria-label="Resumen de resultados filtrados"
        >
          {currencySummary.map(([currencyCode, amount]) => (
            <span key={currencyCode}>
              {currencyCode} {amount}
            </span>
          ))}
          {statusSummary.map(([requestStatus, count]) => (
            <span key={requestStatus}>
              {getReviewStatusLabel(requestStatus)}: {count}
            </span>
          ))}
        </div>
      )}
