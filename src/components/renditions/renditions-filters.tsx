...
function getChips(
  ...
  if (filters.status)
    chips.push({
      key: "status",
      label: `Estado: ${getRenditionStatusLabel(filters.status)}`,
      patch: { status: undefined },
    });
  if (filters.deadline_bucket)
    chips.push({
      key: "deadline-bucket",
      label: `Plazo: ${DEADLINE_BUCKET_LABEL[filters.deadline_bucket]}`,
      patch: { deadline_bucket: undefined },
    });
  if (filters.deadline_from || filters.deadline_to)
    chips.push({
      key: "deadline-range",
      label: `Fecha límite: ${filters.deadline_from ?? "inicio"} – ${filters.deadline_to ?? "hoy"}`,
      patch: { deadline_from: undefined, deadline_to: undefined },
    });
  const sort = RENDITION_SORT_OPTIONS.find(
    (option) => option.value === filters.sort,
  );
  if (sort && filters.sort !== "last_activity")
    chips.push({
      key: "sort",
...
export function RenditionsFilters({
  filters,
  isManager,
  isOperational,
  count,
  ...
  function applyDeadlineRange(): void {
    if (deadlineFrom && deadlineTo && deadlineFrom > deadlineTo) {
      setError(
        "La fecha límite desde no puede ser posterior a la fecha hasta.",
      );
      return;
    }
    setError(null);
    ...
    >
      {isOperational && (
        <GiofWorkScopeFilter
          ...
          helpContext={GIOF_HELP_CONTEXT.REXAN}
        />
      )}
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_15rem_15rem]">
        ...
        >
          <label className="sr-only" htmlFor="renditions-search">
            Buscar rendiciones
          </label>
          <Input
            id="renditions-search"
            value={search}
            maxLength={200}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Código, concepto, solicitante o área"
          />
        ...
        <div>
          <label className="sr-only" htmlFor="renditions-status-filter">
            Estado derivado de rendición
          </label>
          <Select
            ...
            onValueChange={(value) =>
              onChange({
                status:
                  value === ALL_VALUE
                    ? undefined
                    : (value as RenditionsQueueUrlFilters["status"]),
              })
            }
          >
            <SelectTrigger
              id="renditions-status-filter"
              data-testid="renditions-status-filter"
            >
        ...
        <label className="space-y-1 text-sm">
          <span className="sr-only">Plazo de rendición</span>
          <select
            className="h-9 w-full rounded-md border bg-background px-3"
            aria-label="Plazo de rendición"
            value={filters.deadline_bucket ?? ALL_VALUE}
            onChange={(event) =>
              onChange({
                deadline_bucket:
                  event.target.value === ALL_VALUE
                    ? undefined
                    : (event.target
                        .value as RenditionsQueueUrlFilters["deadline_bucket"]),
              })
            }
          >
            <option value={ALL_VALUE}>Todos los plazos</option>
            {Object.entries(DEADLINE_BUCKET_LABEL).map(([value, label]) => (
      ...
      <details className="rounded-md border bg-background p-3">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="space-y-1 text-sm">
            ...
            >
              {RENDITION_SORT_OPTIONS.map((option) => (
              ...
              onChange={(event) =>
                onChange({
                  direction: event.target
                    .value as RenditionsQueueUrlFilters["direction"],
                })
              }
            >
              {RENDITION_DIRECTION_OPTIONS.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
