...
function getChips(
  ...
  if (
    ...
    const label =
      filters.work_scope === GIOF_WORK_SCOPE.MINE
        ? "Trabajo: mi trabajo"
    ...
    chips.push({
      key: "work",
      label,
      patch: { work_scope: undefined, assignee_id: undefined },
    });
  }
  if (filters.search)
    chips.push({
  ...
  if (filters.approved_from || filters.approved_to)
    chips.push({
      key: "approved",
      label: `Aprobación: ${filters.approved_from ?? "inicio"} – ${filters.approved_to ?? "hoy"}`,
      patch: { approved_from: undefined, approved_to: undefined },
    });
  if (filters.paid_from || filters.paid_to)
    chips.push({
      key: "paid",
      label: `Pago: ${filters.paid_from ?? "inicio"} – ${filters.paid_to ?? "hoy"}`,
      patch: { paid_from: undefined, paid_to: undefined },
    });
  if (filters.rejected_from || filters.rejected_to)
    chips.push({
      key: "rejected",
      label: `Rechazo de pago: ${filters.rejected_from ?? "inicio"} – ${filters.rejected_to ?? "hoy"}`,
      patch: { rejected_from: undefined, rejected_to: undefined },
    });
  if (filters.source_account_key)
    chips.push({
      key: "source",
      label: `Cuenta: ${SOURCE_ACCOUNT_LABEL[filters.source_account_key]}`,
      patch: { source_account_key: undefined },
    });
  if (
    filters.completeness &&
    !(
      filters.tab === PAYMENT_QUEUE_TAB.PENDING_DATA &&
      filters.completeness === PAYMENT_COMPLETENESS.ANY_MISSING
    )
  )
    chips.push({
      key: "completeness",
      label: `Completitud: ${COMPLETENESS_LABEL[filters.completeness]}`,
      patch: { completeness: undefined },
    });
  if (filters.drive_status)
    chips.push({
      key: "drive",
      label: formatDriveProjectionStatus(filters.drive_status),
      patch: { drive_status: undefined },
    });
  if (filters.rexan_status)
    chips.push({
      key: "rexan",
      label: getPaymentRexanStatusLabel(filters.rexan_status, false),
      patch: { rexan_status: undefined },
    });
  if (filters.currency) {
    const range = [filters.amount_min, filters.amount_max]
      .filter(Boolean)
      .join(" – ");
    chips.push({
      key: "amount",
      label: range
        ? `Monto pagable ${filters.currency}: ${range}`
  ...
  if (filters.sort && filters.sort !== PAYMENT_QUEUE_SORT.QUEUE_DATE_DESC)
    chips.push({
      key: "sort",
      label: `Orden: ${SORT_LABEL[filters.sort]}`,
...
export function PaymentQueueFilters({
  ...
  const [rejectedFrom, setRejectedFrom] = useState(filters.rejected_from ?? "");
  const [rejectedTo, setRejectedTo] = useState(filters.rejected_to ?? "");
  ...
  useEffect(() => {
    ...
    setRejectedFrom(filters.rejected_from ?? "");
    setRejectedTo(filters.rejected_to ?? "");
    setCurrency(filters.currency ?? "");
    setAmountMin(filters.amount_min ?? "");
    setAmountMax(filters.amount_max ?? "");
  }, [
    filters.approved_from,
    filters.approved_to,
    filters.paid_from,
    filters.paid_to,
    filters.rejected_from,
    filters.rejected_to,
    filters.currency,
    filters.amount_min,
    filters.amount_max,
  ...
  function applyRanges(): void {
    if (
      ...
      (rejectedFrom && rejectedTo && rejectedFrom > rejectedTo) ||
      !isValidMoneyRange(currency, amountMin, amountMax)
    ) {
      setError(
        "Revisa los rangos de fechas y el monto pagable. Los montos requieren moneda y dos decimales.",
      );
      return;
    }
    setError(null);
    onChange({
      ...
      rejected_from: rejectedFrom || undefined,
      rejected_to: rejectedTo || undefined,
      currency: currency
        ? (currency as PaymentQueueUrlFilters["currency"])
        : undefined,
  ...
  const liveMessage = isLoading
    ? "Cargando pagos"
    : isRefreshing
      ? `Actualizando ${count} pagos`
      : `${count} resultados filtrados`;
    ...
    >
      {isManager && (
        <GiofWorkScopeFilter
          ...
          helpContext={GIOF_HELP_CONTEXT.PAYMENT}
        />
      )}
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_15rem_15rem]">
        ...
        >
          <label className="sr-only" htmlFor="payment-queue-search">
            Buscar pagos
          </label>
          <Input
            id="payment-queue-search"
            value={search}
            maxLength={200}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Código, concepto, beneficiario o referencia"
          />
        ...
        <label className="space-y-1 text-sm">
          <span className="sr-only">Completitud del pago</span>
          <select
            className="h-9 w-full rounded-md border bg-background px-3"
            aria-label="Completitud del pago"
            disabled={
              filters.tab === PAYMENT_QUEUE_TAB.PENDING_DATA ||
              filters.tab === PAYMENT_QUEUE_TAB.REJECTED
            }
            value={
              filters.tab === PAYMENT_QUEUE_TAB.PENDING_DATA
              ...
              onChange({
                completeness:
                  event.target.value === "ALL"
                    ? undefined
                    : (event.target
                        .value as PaymentQueueUrlFilters["completeness"]),
              })
            }
          >
            <option value="ALL">Toda completitud</option>
            {Object.entries(COMPLETENESS_LABEL).map(([value, label]) => (
        ...
        <label className="space-y-1 text-sm">
          <span className="sr-only">Orden de pagos</span>
          <select
            className="h-9 w-full rounded-md border bg-background px-3"
            aria-label="Orden de pagos"
            value={filters.sort ?? PAYMENT_QUEUE_SORT.QUEUE_DATE_DESC}
            onChange={(event) =>
              onChange({
                sort: event.target.value as PaymentQueueUrlFilters["sort"],
              })
            }
          >
      ...
      <details className="rounded-md border bg-background p-3">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="space-y-1 text-sm">
            Pago rechazado desde
            <Input
              type="date"
              value={rejectedFrom}
              onChange={(event) => setRejectedFrom(event.target.value)}
            />
          </label>
          <label className="space-y-1 text-sm">
            Pago rechazado hasta
            <Input
              type="date"
              value={rejectedTo}
              onChange={(event) => setRejectedTo(event.target.value)}
            />
          </label>
          <label className="space-y-1 text-sm">
            Cuenta de origen
            <select
              ...
              value={filters.source_account_key ?? "ALL"}
              onChange={(event) =>
                onChange({
                  source_account_key:
                    event.target.value === "ALL"
                      ? undefined
                      : (event.target
                          .value as PaymentQueueUrlFilters["source_account_key"]),
                })
              }
            >
              <option value="ALL">Todas las cuentas</option>
              {Object.entries(SOURCE_ACCOUNT_LABEL).map(([value, label]) => (
                <option key={value} value={value}>
                  {label}
                </option>
              ))}
          ...
          <label className="space-y-1 text-sm">
            ...
            <select
              className="block h-9 w-full rounded-md border bg-background px-3"
              value={filters.drive_status ?? "ALL"}
              onChange={(event) =>
                onChange({
                  drive_status:
                    event.target.value === "ALL"
                      ? undefined
                      : (event.target
                          .value as PaymentQueueUrlFilters["drive_status"]),
                })
              }
            >
              <option value="ALL">Todos los estados Drive</option>
              {Object.values(DRIVE_PAYMENT_PROJECTION_STATUS).map((value) => (
                <option key={value} value={value}>
                  {formatDriveProjectionStatus(value)}
                </option>
              ))}
            </select>
          </label>
          <label className="space-y-1 text-sm">
            ...
            <select
              className="block h-9 w-full rounded-md border bg-background px-3"
              value={filters.rexan_status ?? "ALL"}
              onChange={(event) =>
                onChange({
                  rexan_status:
                    event.target.value === "ALL"
                      ? undefined
                      : (event.target
                          .value as PaymentQueueUrlFilters["rexan_status"]),
                })
              }
            >
              <option value="ALL">Todos los estados REXAN</option>
              {Object.values(PAYMENT_REXAN_STATUS).map((value) => (
                <option key={value} value={value}>
                  {formatRexanActivationStatus(value)}
                </option>
              ))}
            </select>
          </label>
          <label className="space-y-1 text-sm">
            ...
            >
              ...
              {Object.values(REQUEST_CURRENCY).map((value) => (
                <option key={value} value={value}>
                  {value}
                </option>
              ))}
        ...
        <Button type="button" size="sm" className="mt-3" onClick={applyRanges}>
          Aplicar rangos
        </Button>
      </details>
      <QueueFilterChips
